...
const EmptyState = ({ icon: Icon, title, message, action }) => {
    ...
    return (
        <div className="min-h-[500px] flex flex-col items-center justify-center px-4">
            {Icon && <Icon size={64} className="mb-4 text-slate-400 dark:text-gray-500" />}
            ...
            {action && (
                <Link
                    to={action.path}
                    className="mt-6 flex gap-2 items-center px-5 py-2.5 rounded-md bg-blue-500 hover:bg-blue-600 text-white transition font-medium"
                >
